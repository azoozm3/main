...
import {
  consumeCapturedPayment,
  saveCapturedPaymentSession,
  saveSession,
  ...
  app.patch("/api/service-settings", requireAdmin, async (req, res) => {
    try {
      ...
      res
        .status(400)
        .json({
          message: error.message || "Failed to update service settings",
        });
    }
  });
  ...
  app.post("/api/payments/orders", requireAuth, async (req, res) => {
    try {
      if (req.session.userRole !== "patient") {
        return res
          .status(403)
          .json({ message: "Only patients can start a payment" });
      }

      const { serviceKey, referenceId } = createOrderSchema.parse(
        req.body || {},
      );
  ...
  app.post("/api/payments/capture", requireAuth, async (req, res) => {
    try {
      if (req.session.userRole !== "patient") {
        return res
          .status(403)
          .json({ message: "Only patients can capture a payment" });
      }
